...
import com.school.exception.DuplicateEnrollmentException;
import java.util.*;

/**
 * Student entity — extends Person.
 * OOP Layer: Inheritance, Encapsulation
 * Collections Layer: HashMap for grades, HashSet for courses
 */
public class Student extends Person {
    ...
    /** Enroll in a course — throws DuplicateEnrollmentException if already enrolled */
    ...
    public double calculateGPA(int total, int present) {
        ...
        if (!formativeScores.isEmpty()) {
            for (double s : formativeScores.values()) fAvg += s;
            fAvg /= formativeScores.size();
        }
        double att = (total == 0) ? 0 : ((double) present / total) * 100;
        return (summativeScore == 0)
                ? (fAvg * 0.8 + att * 0.2)
                : (fAvg * 0.4 + summativeScore * 0.5 + att * 0.1);
    }
    ...
    public void displayReport(int total, int present) {
        ...
        System.out.printf("   GPA   : %.2f%% | Status: %s%n", gpa, (gpa >= 50 ? "PASSING" : "AT RISK"));
    }

    // --- Getters ---
    public String getStudentId()               { return studentId; }
    public Map<String, Double> getFormativeScores() { return formativeScores; }
    public Set<String> getEnrolledCourseNames()     { return enrolledCourseNames; }
    public double getSummativeScore()              { return summativeScore; }
